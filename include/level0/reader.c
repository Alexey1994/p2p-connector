...
void read_next_byte(Reader* reader)
{
	...
	if(reader->queue.size) {
		read_bytes_from_queue(&reader->queue, &bytes, sizeof(bytes));
	}
	else {
		...
		if(bytes_readed < 0) {
			...
			write_bytes_in_queue(&reader->queue, &bytes, sizeof(bytes));
		}
	}
}
...
Byte get_reader_byte(Reader* reader)
{
	...
	return reader->queue.data[reader->queue.start_index];
}
...
Number get_reader_bytes(Reader* reader, Byte* bytes, Number number_of_bytes)
{
	...
	while(number_of_bytes && !end_of_reader(reader)) {
...
Real_Number32 read_Real_Number(Reader* reader)
{
	...
	Real_Number32 basis;
	Signed_Number exponent;
	Real_Number32 multiplier;
	...
	if(get_reader_byte(reader) == 'e') {
		read_next_byte(reader);
		
		exponent = read_Signed_Number(reader);
	...
	if(is_negative) {
		return -((decimal + fractional) * multiplier);
	}
	else {
		return (decimal + fractional) * multiplier;
	}
