#ifndef WINAPI_KERNEL32_INCLUDED
...
Number32 GetLastError();
void     SetLastError(Number32 error_code);
...
typedef struct {
	Number   status;
	Number   number_of_transferred_bytes;
	Number32 offset;
	Number32 offset_high;
	void*    event;
}
...
import stdcall Boolean CreatePipe(Byte* read, Byte* write, SECURITY_ATTRIBUTES* security_attributes, Number32 buffer_size);
...
Boolean GetCurrentDirectoryW(Number32 path_size_in_chars, Number16* path);
Boolean SetCurrentDirectoryW(Number16* path);
...
import stdcall Number32 GetFullPathNameW(Number16* path, Number32 full_path_size, Number16* full_path, Number16* part_path);
import stdcall Number32 GetFinalPathNameByHandleW(Byte* file, Number16* final_path, Number32 path_size, Number32 flags);
...
import stdcall Boolean CreateHardLinkW(Number16* synonym_name, Number16* existing_name, void* zero);
import stdcall Boolean CreateSymbolicLinkW(Number16* synonym_name, Number16* existing_name, Number32 flags);
...
import stdcall Byte ReadFileEx(
	File        file,
	Byte*       buffer,
	Number32    buffer_length,
	Number32*   bytes_readed,
	Overlapped* overlapped,
	stdcall void (*completion_routine)(Number32 error_code, Number32 number_of_transferred_bytes, Overlapped* overlapped)
);
...
import stdcall Boolean GetOverlappedResult(File file, Byte* overlapped, Number32* number_of_bytes, Boolean wait);
...
#define IO_REPARSE_TAG_SYMBOLIC_LINK 0x00000000
#define IO_REPARSE_TAG_MOUNT_POINT   0x00000003
#define IO_REPARSE_TAG_WIM           0x00000008
#define IO_REPARSE_TAG_SYMLINK       0x0000000C
//773107F3
...
import stdcall Byte*   FindFirstFileNameW(Number16* file_name, Number32 flags, Number32* name_size, Number16* name);
import stdcall Boolean FindNextFileNameW(Byte* current_state, Number32* name_size, Number16* name);

import stdcall Boolean GetFileSizeEx(File file, Number64* size);
import stdcall Number32 GetFileAttributesW(Number16* file_path);
...
import stdcall Boolean ReadDirectoryChangesW(
	...
	Number32*          bytes_returned,
	Overlapped*        overlapped,
	stdcall void     (*notify)(Number32 error_code, Number32 bytes_transfered, Overlapped* overlapped)
);
...
import stdcall void GetStartupInfoW(Sturtup_Info* info);
...
import stdcall Number32 CreateThread (
	...
	Number                       stack_size,
	stdcall void               (*thread_function)(Byte* arguments),
	Byte*                        attributes,
	Number32                     creation_flags,
	Number32*                    thread_id
);

import stdcall Boolean TerminateThread(Byte* thread, Number32 exit_code);
